...
public final class RabbitPublishingExternalAsyncSentimentClassifier implements AsyncSentimentClassifier {

    private static final Logger log = LoggerFactory.getLogger(RabbitPublishingExternalAsyncSentimentClassifier.class);
    ...
    public RabbitPublishingExternalAsyncSentimentClassifier(RabbitTemplate rabbitTemplate,
                                                            String exchange,
    ...
    public void submit(long reviewId, String text) {
        ...
        try {
            ...
            log.warn("X-Async publish to broker failed for review {}: {}", reviewId, e.getMessage());
            throw new SentimentClassificationException(
                    FailureMode.UNREACHABLE,
                    "Broker publish failed for review " + reviewId,
                    e);
        } catch (RuntimeException e) {
            log.error("Unexpected error publishing review {} (X-Async)", reviewId, e);
            throw new SentimentClassificationException(
                    FailureMode.UNKNOWN,
        ...
        if (confirmed == null || !confirmed) {
            log.warn("X-Async publisher confirm for review {} returned negative or timed out after {} ms",
