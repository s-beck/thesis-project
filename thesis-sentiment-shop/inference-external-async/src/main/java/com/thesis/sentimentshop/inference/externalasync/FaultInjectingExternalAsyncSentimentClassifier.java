...
public final class FaultInjectingExternalAsyncSentimentClassifier implements AsyncSentimentClassifier, FaultInjectionControl {
    ...
    private final AsyncSentimentClassifier delegate;
    ...
    public FaultInjectingExternalAsyncSentimentClassifier(AsyncSentimentClassifier delegate,
    ...
    public void scheduleFailures(FailureMode mode, int count) {
        ...
        if (!supportedModes.contains(mode)) {
            state.set(InjectionState.skipped(mode,
                    mode + " is structurally applicable to " + variantName
                            + " but originates after submit() returns and "
                            + "cannot be injected from this side of the broker"));
            return;
        }
